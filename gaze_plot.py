...
def draw_heatmap(gazepoints, dispsize, imagefile=None, alpha=0.5, savefilename=None, gaussianwh=200, gaussiansd=None):
    ...
    # heatmapsize = math.floor(dispsize[1] + 2 * strt), math.floor(dispsize[0] + 2 * strt)
...
def displayHeatmap(array, display_width, display_height, alpha, output_name, background_image, ngaussian, sd):
    gaza_data = []
    if len(array[0]) is 2:
        gaze_data = list(map(lambda q: (int(q[0]), int(q[1]), 1), array))
    else:
        gaze_data =  list(map(lambda q: (int(q[0]), int(q[1]), int(q[2])), array))
